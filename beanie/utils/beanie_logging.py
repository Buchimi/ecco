...
class LogLevel(Enum):
    """Utility enum that keeps the types of logs and their respective color"""
    NONE = ("", ANSI_RESET)
...
def log(level: LogLevel, message: str, override_category_str: str = ""):
    ...
    category_str = override_category_str if override_category_str != "" else str(
        level)
...
class BeanieInternalTypeError(BeanieFatalException):
    return_code = 5
