...
from ..beanie import ARGS
from ..parsing import ASTNode
from ..scanning import TokenType
from ..utils import BeanieFatalException, BeanieFileError, LogLevel, log
...
def translate_init():
    ...
    We also create an LLVM out file for writing into
    Raises:
        BeanieFileError: Raised if an error occurs while opening the output LLVM
                       file
    """
...
def determine_binary_expression_stack_allocation(root: ASTNode) -> List[LLVMStackEntry]:
    ...
    if root.left or root.right:
        if root.left:
            left_entry = determine_binary_expression_stack_allocation(
                root.left)
        if root.right:
            right_entry = determine_binary_expression_stack_allocation(
                root.right)
        return left_entry + right_entry
    else:
        out_entry = LLVMStackEntry(
            LLVMValue(LLVMValueType.VIRTUAL_REGISTER, get_next_local_virtual_register()
                      ), 4
        )
...
def ast_to_llvm(root: ASTNode) -> LLVMValue:
    ...
    Returns:
        LLVMValue: LLVMValue containing register number of expression value to
                   print
    """
    from .llvm import (llvm_binary_arithmetic,
                       llvm_ensure_registers_loaded, llvm_store_constant)
