...
from ..utils import BeanieFatalException
...
from .translate import (LLVM_LOADED_REGISTERS,
                        LLVM_OUT_FILE, get_next_local_virtual_register,
...
def llvm_ensure_registers_loaded(
   ...
   """
    found_registers: List[bool] = [False] * \
        len(registers)  # create an arr of false
    ...
    for loaded_register in LLVM_LOADED_REGISTERS:
        for i, reg in enumerate(registers):
            ...
            if found_registers.count(True) == len(registers):
                return registers
    # load unloaded registers
    ...
    for i in range(len(registers)):
        if not found_registers[i]:
            ...
            loaded_registers.append(
                LLVMValue(LLVMValueType.VIRTUAL_REGISTER, new_reg))
            LLVM_OUT_FILE.writelines([
                TAB, f"%{new_reg} = load i32, i32* %{registers[i].int_value}",
                NEWLINE,
            ])
            LLVM_LOADED_REGISTERS.append(loaded_registers[-1])
        else:
            loaded_registers.append(registers[i])
...
def llvm_binary_arithmetic(
    ...
    Raises:
        BeanieFatalException: If the passed Token is not a binary arithmetic
                            operator
    Returns:
